...
public class MarkdownSearchTool(Project project, ITextEmbeddingGenerationService embeddingGenerationService, IVectorStoreRecordCollection<Guid, MarkdownVectorEntity> collection, int numberOfResultsBack, double scoreShouldBeBelowThis, ProgressNotificationBase parent)
{
    [UsedImplicitly]
    [KernelFunction]
    public async Task<string[]> Search(string searchQuery)
    {
        ...
        ProgressNotification notification = new(DateTimeOffset.UtcNow, $"Markdown Search Called ({results.Count} Results)")
        {
